...
void Bilateral::fitsError(int status) {
    if (status) {
...
void Bilateral::processFitsFile(const char* inputFitsPath, const char* outputFitsPath) {
    // 读取FITS文件
    fitsfile* fitsPtr;
    int status = 0;
    ...
    std::cout << "双边滤波处理成功。输出的FITS文件保存在：" << outputFitsPath << std::endl;
